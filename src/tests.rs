...
fn test_util_compress_decompress<TBitPacker: BitPacker>(bitpacker: &TBitPacker, data: &[u32], expected_num_bits: u8) {
...
fn test_util_compress_decompress_delta<TBitPacker: BitPacker>(bitpacker: &TBitPacker, data: &[u32], expected_num_bits: u8) {
    assert_eq!(data.len(), TBitPacker::BLOCK_LEN);
    ...
    for initial in 0u32..2u32 {
        ...
        bitpacker.decompress_sorted(initial, &compressed[..compressed_len], &mut result[..], numbits);
...
pub unsafe fn bench_compress_util<TBitPacker: BitPacker>(bench: &mut Bencher, num_bits_arr: &[u8]) {
    ...
    if let Some(bitpacker) = TBitPacker::acquire() {
        bench.iter(|| {
            let mut offset = 0;
...
pub unsafe fn bench_compress_delta_util<TBitPacker: BitPacker>(bench: &mut Bencher, num_bits_arr: &[u8]) {
    let num_blocks = num_bits_arr.len();
    bench.bytes = (num_blocks * TBitPacker::BLOCK_LEN * 4) as u64;
...
pub unsafe fn bench_decompress_util<TBitPacker: BitPacker>(bench: &mut Bencher, num_bits_arr: &[u8]) {
    let num_blocks = num_bits_arr.len();
    bench.bytes = (num_blocks * TBitPacker::BLOCK_LEN * 4) as u64;
    let original_values = create_array(TBitPacker::BLOCK_LEN, &num_bits_arr, DataType::NoDelta);
 ...
 pub unsafe fn bench_decompress_delta_util<TBitPacker: BitPacker>(bench: &mut Bencher, num_bits_arr: &[u8]) {
    let num_blocks = num_bits_arr.len();
    bench.bytes = (num_blocks * TBitPacker::BLOCK_LEN * 4) as u64;
    let original_values = create_array(TBitPacker::BLOCK_LEN, &num_bits_arr, DataType::Delta(3u32));
    let mut compressed = vec![0u8; original_values.len() * 4];
    ...
    if let Some(bitpacker) = TBitPacker::acquire() {
        ...
        for num_bits in 0u8..32u8 {
            ...
            for i in 0..num_blocks {
                ...
                if delta {
                    test_util_compress_decompress_delta(&bitpacker,block, computed_num_bits);
                } else {
                    test_util_compress_decompress(&bitpacker,block, computed_num_bits);
                }
            }
        }
    }
