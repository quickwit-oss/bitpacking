                ...
                unsafe {
                    ...
                    unroll! {
                        for iter in 0..30 {
                            ...
                            let in_register: DataType = load_unaligned(input_ptr.offset(i as isize));

                            out_register =
                                if inner_cursor > 0 {
                                    op_or(out_register, left_shift_32(in_register, inner_cursor as i32))
                                } else {
                                    in_register
                                };
                            ...
                            if inner_capacity <= NUM_BITS {
                                ...
                                if inner_capacity < NUM_BITS {
                                    out_register = right_shift_32(in_register, inner_capacity as i32);
                                }
                            }
                        }
                    }
                    ...
                    out_register =
                        if NUM_BITS != 32 {
                            op_or(out_register, left_shift_32(in_register, (32 - NUM_BITS) as i32))
                        } else {
                            in_register
                        };
                    store_unaligned(output_ptr, out_register);
                    ...
                    unroll! {
                        for iter in 0..31 {
                            ...
                            if inner_capacity <= NUM_BITS && i != 31 {
                                ...
                                if inner_capacity < NUM_BITS {
                                    out_register = op_or(out_register, op_and(left_shift_32(in_register, inner_capacity as i32), mask))
                                }
                            }
