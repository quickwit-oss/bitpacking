...
mod scalar {
    ...
    fn integrate_delta(offset: DataType, delta: DataType) -> DataType {
        ...
        let el4 = el2.wrapping_add(delta[4]);
        let el5 = el2.wrapping_add(delta[5]);
        let el6 = el2.wrapping_add(delta[6]);
        let el7 = el2.wrapping_add(delta[7]);
...
impl BitPacker for BitPacker8x {
        ...
        unsafe {
            #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
            {
                if is_x86_feature_detected!("avx2") {
                    return avx2::compress(decompressed, compressed, num_bits);
                }
            }
    ...
    fn compress_sorted(initial: u32, decompressed: &[u32], compressed: &mut [u8], num_bits: u8) -> usize {
        unsafe {
            #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
                {
                    if is_x86_feature_detected!("avx2") {
                        return avx2::compress_sorted(initial, decompressed, compressed, num_bits);
                    }
                }
    ...
    fn decompress(compressed: &[u8], decompressed: &mut [u32], num_bits: u8) -> usize {
        unsafe {
            ...
            {
                if is_x86_feature_detected!("avx2") {
                    return avx2::decompress(compressed, decompressed, num_bits);
                }
            }
    ...
    fn decompress_sorted(initial: u32, compressed: &[u8], decompressed: &mut [u32], num_bits: u8) -> usize {
        unsafe {
            ...
            {
                if is_x86_feature_detected!("avx2") {
                    return avx2::decompress_sorted(initial, compressed, decompressed, num_bits);
                }
            }
    ...
    fn num_bits(decompressed: &[u32]) -> u8 {
        unsafe {
            ...
            {
                if is_x86_feature_detected!("avx2") {
                    return avx2::num_bits(decompressed);
                }
            }
    ...
    fn num_bits_sorted(initial: u32, decompressed: &[u32]) -> u8 {
        unsafe {
            ...
            {
                if is_x86_feature_detected!("avx2") {
                    return avx2::num_bits_sorted(initial, decompressed);
                }
            }
